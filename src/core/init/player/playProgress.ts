...
export default () => {
  ...
  let seekOverrideUntil = 0
  let seekOverrideTime = 0
  const getCurrentTime = () => {
    const id = playerState.musicInfo.id
    void getPosition().then(position => {
      if (Date.now() < seekOverrideUntil) {
        setNowPlayTime(seekOverrideTime)
        return
      }
  ...
  const setProgress = (time: number, maxTime?: number) => {
    ...
    seekOverrideTime = time
    seekOverrideUntil = Date.now() + 1500
    setNowPlayTime(time)
