...
const getMusicPlayUrl = async(musicInfo: LX.Music.MusicInfo | LX.Download.ListItem, isRefresh = false, isRetryed = false): Promise<string | null> => {
  ...
  // 锁屏/Doze 下网络请求可能长期挂起，加超时兜底，避免播放永久卡在获取 URL。
  // 45s：容纳完整解析链路（内部 API 多源轮询 + 音质降级 + 跨平台换源）
  const timeoutMs = 45_000
  let timeoutTimer: number | null = null
  const withTimeout = (promise: Promise<string>) => Promise.race([
    promise,
    new Promise<string>((_, reject) => {
      // 后台时 JS setTimeout 不可靠，用 BackgroundTimer 保证超时触发
  ...
  const fetchUrl = () => (toggleMusicInfo ? getMusicUrl({
  ...
  return withTimeout(fetchUrl()).then(url => {
...
export const setMusicUrl = (musicInfo: LX.Music.MusicInfo | LX.Download.ListItem, isRefresh?: boolean) => {
  ...
  void getMusicPlayUrl(musicInfo, isRefresh).then((url) => {
    if (!url) return
    // 提前清除 gettingUrlId，让 PlaybackState 的 Playing 事件能被处理
...
// 播放错误兜底（照抄 Web 播放器 audio error retryHandler）：
// 播放失败时丢弃坏缓存链接 → 强制重新在线解析（内部 API 多源轮询 → 音质降级 → 跨平台换源），
// 连续失败 N 次后自动切到下一首，避免单曲卡死。
let playErrorCount = 0
export const resetPlayErrorCount = () => {
  playErrorCount = 0
}
export const handlePlaybackError = async(message?: string) => {
  const playMusicInfo = playerState.playMusicInfo.musicInfo
  if (!playMusicInfo || global.lx.isPlayedStop || global.lx.isChangingMusic) return
  ...
  // 丢弃坏的缓存链接，重新走完整解析链路（isRefresh 跳过缓存）
...
export const collectMusic = () => {
  ...
  void toggleFavorite(musicInfo as LX.Music.MusicInfoOnline)
}
