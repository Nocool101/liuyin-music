...
export const initSetting = async() => {
  ...
  // 旧默认值 false 会导致排行榜/歌单点歌时按"默认歌单"播放（播完跳错歌），
  // 本移植版按列表上下文点播是预期行为，一次性强制开启
  if (updatedSetting.setting['list.isClickPlayList'] === false) {
    updatedSetting.setting['list.isClickPlayList'] = true
  }
